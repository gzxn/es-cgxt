...
import com.gzxn.core.system.annotation.AreaMapper;
...
public class AreaMapperHandler implements IResponseAnnotationHandler {
    ...
    public Object handle(Object responseObject, Collection<Field> fieldSets) {
        if (ObjectUtil.isEmpty(iAreaCommonApi)) {
            EbpRunTimeException ebpRunTimeException = new EbpRunTimeException("AreaAspect 注入入库组件出错。");
        ...
        for (Field setField : fieldSets) {
            // 获取属性注解
            AreaMapper areaMapper = setField.getAnnotation(AreaMapper.class);
            // 返回属性名
