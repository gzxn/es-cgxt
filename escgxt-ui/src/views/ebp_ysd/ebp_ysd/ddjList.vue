				...
				<el-table-column label="操作" fixed="right" width="200px">
					<template slot-scope="scope">
						<div class="dataTable-operator">
							<el-button v-has-permission-code-and="'6000022001'" size="mini" icon="el-icon-search"@click="toDetail(scope.$index, scope.row)">详情</el-button>
						    <el-button v-has-permission-code-and="'6000022001'" size="mini" type="primary" icon="el-icon-edit" @click="toUpdate(scope.$index, scope.row)">验收登记</el-button>
	...
	export default {
		...
		methods: {
			...
			toUpdate(index, row) {
				...
				this.$refs.ysdForm.open(editParameter, '验收登记', false, '70%');
			},
			//去详情
			toDetail(index, row) {
				let detailParameter = {
				...
				this.$refs.ysdForm.open(detailParameter, '详情', true, '70%');
			},
			//删除
			remove(index, row) {
				let parameter = {
