	...
	<div class="tab-body">
		<div class="search">
			<el-form ref="form" :model="searchParams" label-width="100px">
				<el-row>
					<el-col :span="6">
						<el-form-item label="采购任务编号">
							<el-input v-model="searchParams.cgrwbh"></el-input>
						</el-form-item>
					</el-col>
					<el-col :span="6">
						<el-form-item label="采购任务名称">
							<el-input v-model="searchParams.cgrwmc"></el-input>
						</el-form-item>
					</el-col>
        ...
        <div class="dataTable autoFlex" v-if="listData">
			<qlh-table class="table" :data="listData.records" stripe border height="auto" @selection-change="selectionChange" 
				...
				<qlh-table-column prop="cgrwbh" label="采购任务编号" width="130"></qlh-table-column>
				<qlh-table-column prop="cgrwmc" label="采购任务名称"></qlh-table-column>
				...
				<qlh-table-column prop="cgzxryhm" label="采购执行人用户名"></qlh-table-column>
				<qlh-table-column prop="crtm" label="已分配时间"></qlh-table-column>
				<qlh-table-column label="操作" fixed="right" width="180px">
					<template slot-scope="scope">
						<div class="dataTable-operator">
							<el-button v-has-permission-code-and="'9000022001'" size="mini" icon="el-icon-search"@click="toDetail(scope.$index, scope.row)">详情</el-button>
						    <!-- <el-button v-has-permission-code-and="'9000022001'" size="mini" icon="el-icon-edit" @click="toUpdate(scope.$index, scope.row)">编辑</el-button> -->
							<el-button v-has-permission-code-and="'9000022001'" size="mini" icon="el-icon-edit" type="primary" @click="cgrwPack(scope.$index, scope.row)">打包</el-button>
                            <!-- <el-button v-has-permission-code-and="'2000013005'" size="mini" icon="el-icon-delete" type="danger" @click="remove(scope.$index, scope.row)">删除</el-button> -->
                        </div>
					</template>
				</qlh-table-column>
			</qlh-table>
		...
		<edit ref="edit"></edit>
		<cgbEdit ref="cgbEdit"></cgbEdit>
	</div>
</template>
<script>
	import edit from './edit.vue';
	import cgbEdit from './cgbEdit.vue';
	...
	export default {
		...
		methods: {
			...
			toUpdate(index, row) {
				...
				this.$refs.edit.open(editParameter, '分配采购任务', false, '70%');
			},
			//去详情
			toDetail(index, row) {
				let detailParameter = {
				...
				this.$refs.cgbEdit.open(detailParameter, '详情', true, '70%');
			},
			//去打包
			cgrwPack(index, row) {
				let packParameter = {
				...
				// this.$refs.edit.open(editParameter, '编辑');
				// this.$refs.edit.open(packParameter, '打包', false, '70%');
				this.$utils.confirm.warning('提示','确定打包吗？', () => {
					zbxmApi.cgrwPack(
						packParameter, 
						response => {
