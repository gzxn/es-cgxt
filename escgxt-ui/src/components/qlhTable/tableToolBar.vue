<template>
	<div style="position: relative;">
		<div style="width: 100%;height:15px;cursor: pointer;text-align: center;" @click="showToolBar=!showToolBar">
			<i :class="showToolBar?'el-icon-arrow-down':'el-icon-arrow-up'" style="color: #b0b1b4;"></i>
		</div>
		<el-collapse-transition>
			<div style="width: 100%;height: 30px;border: 1px solid #EBEEF5;" :style='{"text-align":toolButtonDirection}' v-if="showToolBar">
...
<script>
	...
	export default {
		...
		methods: {
			...
			exportExcel() {
				this.initTable();
				this.$utils.file.generateExcel(document.querySelector("#" + this.tableId), this.excelProp);
			},
			print() {
				this.initTable();
			...
			printHTML(id) {
				...
				this.$utils.print.printHtml(printDOMID, this.printProp);
				div.remove();
			},
			init() {}
