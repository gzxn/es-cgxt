<template>
	<div style="display: flex;">
		...
		<jfly-table-dialog ref="jflyTable" :multiple="multiple" @confirm="confirm"></jfly-table-dialog>
	</div>
</template>

<script>
	import jflyTableDialog from '@/components/biz/selectJflyTableDialog/jflyTableDialog.vue';
	import api from '@/api/ebp_jfly/ebp_jfly/ebp_jfly.js';
	export default {
		name: 'selectJflyTableDialog',
		components: {
			jflyTableDialog,
		},
		data() {
		...
		methods: {
			...
			open() {
				this.$refs.jflyTable.open(this.selectedListData, this.placeholder, '95%');
			},
			removeTag(value) {
